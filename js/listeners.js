...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    setupAvatarFrameSettings();
            ...
            function updateDelayUI() {
                ...
                // ====== 新增部分：同步衰减系数显示 ======
                const ds = document.getElementById('reply-delay-decrement-slider');
                const dsv = document.getElementById('reply-delay-decrement-value');
                if (ds) ds.value = (typeof settings.replyDelayDecrement !== 'undefined') ? settings.replyDelayDecrement : 0.9;
                if (dsv) dsv.textContent = parseFloat(ds.value).toFixed(2);
                // ========================================
            }
            updateDelayUI();
            ...
            // ====== 新增部分：绑定衰减系数滑动条事件 ======
            const decrementSlider = document.getElementById('reply-delay-decrement-slider');
            const decrementVal = document.getElementById('reply-delay-decrement-value');
            if (decrementSlider) {
                // 滑动时实时更新旁边显示的数字
                decrementSlider.addEventListener('input', (e) => {
                    if (decrementVal) decrementVal.textContent = parseFloat(e.target.value).toFixed(2);
                });
                // 松开鼠标时保存设置
                decrementSlider.addEventListener('change', (e) => {
                    settings.replyDelayDecrement = parseFloat(e.target.value);
                    throttledSaveData();
                });
            }
            // ============================================
